...
export function ProgressView({ lvlIdx, setLvlIdx, setSecIdx, setTab, checked, key_fn, toggle, levelStats, examResults }: ProgressViewProps) {
  const handleSetLvlIdx = (li: number) => {
    if (canAccessLevel(li, examResults)) {
      setLvlIdx(li);
      setSecIdx(0);
      setTab("content");
    }
          ...
          {LEVELS.map((lv, li) => {
            ...
            if (isLocked) {
              return (
                <div
                  key={li}
                  className="cursor-not-allowed px-4 py-3 rounded-md min-w-[160px] flex-1 opacity-50"
                  style={{
                ...
                >
                  <div className="flex items-center gap-1 mb-1">
                    ...
                    <span className="text-[11px] tracking-widest" style={{ color: lv.color }}>
                      NIVEL {lv.id}
                    </span>
                  </div>
