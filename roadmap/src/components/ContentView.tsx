...
import type { Level, Section, ProgressStats, ExamResult } from "../types";
...
export function ContentView({ section, level, lvlIdx, secIdx, openItem, setOpenItem, checked, key_fn, toggle, examResults }: ContentViewProps) {
  const isLocked = !canAccessLevel(lvlIdx, examResults);

  if (isLocked) {
    const requirements = getUnlockRequirements(lvlIdx);
    return (
